...
public class ReportPortalReporter : BaseReporter
{
    private static readonly IDictionary<
        AssertionStatus,
        Status
    > AssertionStatusToReportPortalStatusMap = new Dictionary<AssertionStatus, Status>
    {
    ...
    private static readonly IDictionary<
        AssertionSeverity,
        string
    > AssertionSeverityToAttributeValueMap = new Dictionary<AssertionSeverity, string>
    {
    ...
    internal void PublishQueuedResults(
        ReportPortalPublishContext publishContext,
    ...
    private void PublishTestResultCore(
        ReportPortalPublishContext launch,
        AssertionResult assertionResult
    )
    {
        var requestedStartTimeUtc = GetAssertionStartTime(assertionResult);
        var startTimeUtc =
            requestedStartTimeUtc < launch.LaunchStartTimeUtc
                ? launch.LaunchStartTimeUtc
                : requestedStartTimeUtc;
        var finishTimeUtc = startTimeUtc.AddMilliseconds(
            Math.Max(assertionResult.TestDurationMs, 1)
        );
        var launchPlan = launch.LaunchPlan;
        var itemAttributes = BuildItemAttributes(assertionResult, launchPlan);
        var stableIdentity = BuildStableReportPortalIdentity(
            assertionResult,
            launchPlan.Team,
            launchPlan.System
        );
        var itemUuid = launch
            .Service.TestItem.StartAsync(
        ...
        try
        {
            ...
            launch
                .Service.TestItem.FinishAsync(
                    itemUuid,
                    new FinishTestItemRequest
                    {
                        LaunchUuid = launch.LaunchUuid,
                        EndTime = finishTimeUtc,
                        Status = AssertionStatusToReportPortalStatusMap[
    ...
    private void WriteAssertionContextLog(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult,
    ...
    )
    {
        ...
        var text = new StringBuilder()
            ...
            .AppendLine()
            .AppendLine(
                string.IsNullOrWhiteSpace(assertionTextDetails.Message)
        ...
        {
            text.AppendLine().AppendLine(assertionTextDetails.Trace);
        }

        CreateLogItem(launch, itemUuid, logLevel, text.ToString().Trim(), null);
    }

    private void WriteLinksLog(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult
    )
    ...
    private void WriteSessionDetails(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult
    )
        ...
        foreach (var sessionData in assertionResult.Assertion.SessionDataList)
        {
            ...
            foreach (var actionFailure in actionFailures)
            {
                CreateLogItem(
                    launch,
                    itemUuid,
                    ReportPortalLogLevel.Error,
                    BuildActionFailureText(sessionData, actionFailure),
                    null
                );
            }
        }
    ...
    private void WriteSessionLogAttachments(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult
    )
    {
        foreach (var sessionData in assertionResult.Assertion.SessionDataList)
        {
            var sessionLogArtifact = BuildSessionLogArtifact(
                sessionData,
    ...
    private void WriteTemplateAttachment(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult
    )
    {
        ...
        CreateLogItem(
            ...
            itemUuid,
            ReportPortalLogLevel.Info,
            "Execution configuration template.",
            templateArtifact
        );
    }

    private void WriteAssertionAttachments(
        ReportPortalPublishContext launch,
        string itemUuid,
        AssertionResult assertionResult
    )
    ...
    )
    {
        ...
        if (artifact is not null)
        {
            request.Attach = new LogItemAttach(
                artifact.ContentType ?? RawDataAttachmentType,
                artifact.Content
            )
            {
    ...
    private static DateTime GetAssertionStartTime(AssertionResult assertionResult)
    {
        if (assertionResult.Assertion.SessionDataList.Any())
            return assertionResult.Assertion.SessionDataList.Min(sessionData =>
    ...
    private IList<KeyValuePair<string, string>> BuildParameters(
        AssertionResult assertionResult,
    ...
    )
    {
        ...
        foreach (
            var metadataAttribute in BuildMetadataAttributes()
                .Where(attribute => !string.IsNullOrWhiteSpace(attribute.Key))
        )
        {
            parameters.Add(
                new KeyValuePair<string, string>(
                    metadataAttribute.Key.Trim(),
    ...
    private IList<ItemAttribute> BuildItemAttributes(
        AssertionResult assertionResult,
        ReportPortalLaunchPlan launchPlan
    )
    {
        ...
        {
            new() { Key = "tool", Value = QaaSTag },
            new() { Key = "assertion", Value = assertionResult.Assertion.AssertionName },
            new()
            {
                Key = "severity",
                Value = AssertionSeverityToAttributeValueMap[
                    ResolveSeverity(assertionResult.Assertion)
                ],
            },
            new() { Key = "team", Value = launchPlan.Team },
            new() { Key = "project", Value = launchPlan.Project },
            new() { Key = "system", Value = launchPlan.System },
        };

        foreach (
            var sessionName in assertionResult
                .Assertion.SessionDataList.Select(session => session.Name)
        ...
        foreach (
            var attribute in launchPlan.Attributes.Where(attribute =>
    ...
    )
    {
        ...
        description
            ...
            .AppendLine(
                $"- Sessions: {string.Join(", ", assertionResult.Assertion.SessionDataList.Select(session => session.Name))}"
            )
            .AppendLine(
                $"- Data Sources: {string.Join(", ", assertionResult.Assertion.DataSourceList?.Select(dataSource => dataSource.Name) ?? [])}"
            );
