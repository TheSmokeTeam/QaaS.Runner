...
public class BootstrapTests
{
    ...
    private static IEnumerable<TestCaseData> GetRunnerTestCases()
    {
        yield return new TestCaseData(
            new string[] { "run", "TestData/test.qaas.yaml","-w", "TestData/override.yaml" },
            "Runner"
        ).SetName("WithRunOptions");

        yield return new TestCaseData(
            new string[] { "act", "TestData/test.qaas.yaml","-w", "TestData/override.yaml" },
            "Runner"
        ).SetName("WithActOptions");

        yield return new TestCaseData(
            new string[] { "assert", "TestData/test.qaas.yaml","-w", "TestData/override.yaml" },
            "Runner"
        ).SetName("WithAssertOptions");

        yield return new TestCaseData(
            new string[] { "template", "TestData/test.qaas.yaml", "-w", "TestData/override.yaml" },
            "Runner"
        ).SetName("WithTemplateOptions");

        yield return new TestCaseData(
            new string[] { "execute", "TestData/executable.yaml"},
            "Runner"
